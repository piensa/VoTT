...
$(document).ready(() => {
  //init confirm keys figure out why this doesn't work
...
ipcRenderer.on("export-records", async (event, exportConfig) => {
  ...
  if (videotagging.imagelist && visitedFrames.size > 0) {
    ...
    switch (exportConfig.exportUntil) {
      case "tagged":
        exportUntil = videotagging.imagelist
          .map(file => file.split(path.sep).pop())
          .indexOf(
            Object.keys(self.videotagging.frames)[
              Object.keys(self.videotagging.frames).length - 1
            ]
          );
        break;
    ...
    let carg = async.queue(async function(tasks, callback) {
      if (videotagging.recordlist)
        await writeRecord(
          videotagging.imagelist[tasks.i],
          videotagging.recordlist[tasks.i],
...
ipcRenderer.on("export-tags", (event, exportConfig) => {
  addLoader();
  detection.export(
    videotagging.imagelist,
    exportConfig.exportFormat,
    exportConfig.exportUntil,
    exportConfig.exportPath,
    testSetSize,
    () => {
...
ipcRenderer.on("review-model", (event, reviewModelConfig) => {
  ...
  if (fs.existsSync(modelLocation)) {
    ...
    detection.review(
      videotagging.imagelist,
      reviewModelConfig.modelFormat,
      modelLocation,
      reviewModelConfig.output,
      err => {
        if (err) {
          alert(
            `An error occured with Local Active Learning \n Please check the debug console for more information.`
          );
    ...
    alert(
      `No model found! Please make sure you put your model in the following directory: ${modelLocation}`
    );
  }
});
...
ipcRenderer.on("review-model-endpoint", (event, reviewModelConfig) => {
  addLoader();
  detection.reviewEndpoint(
    videotagging.imagelist,
    reviewModelConfig.endpoint,
    err => {
      if (err) {
        alert(
          `An error occured with Remote Active Learning \n Please check the debug console for more information.`
        );
      }
      if (!videotagging.imagelist) {
        videotagging.video.oncanplay = updateVisitedFrames;
...
function saveLoggedTime() {
  ...
  if (videotagging && videotagging.src) {
    fs.writeFile(
      `${videotagging.src}.log.json`,
      JSON.stringify(loggedTime, null, "\t"),
      err => {
        if (err) throw err;
...
window.addEventListener(
  ...
  e => {
    if (videotagging) {
      ...
      if (
        e.ctrlKey &&
        (e.code == "KeyC" || e.code == "KeyX" || e.code == "KeyA")
      ) {
        var widthRatio = videotagging.overlay.width / videotagging.sourceWidth;
        var heightRatio =
          videotagging.overlay.height / videotagging.sourceHeight;
        var content = [];
...
function updateVisitedFrames() {
  if (videotagging.imagelist) {
    visitedFrames.add(
      videotagging.imagelist[videotagging.imageIndex].split(path.sep).pop()
    );
...
function openPath(pathName, isDir, isRecords = false) {
  ...
  //set title indicator
  $("title").text(
    `Tagging Job Configuration: ${path.basename(
      pathName,
  ...
  assetFolder = path.join(
    path.dirname(pathName),
    `${path.basename(pathName, path.extname(pathName))}_output`
  );
  ...
  function loadTaggerAndCheckRegions(e) {
    ...
    loadTagger(e).then(() => {
      if (
        videotagging.currTFRecord &&
        videotagging.getCurrentFrameRegions().length == 0
      ) {
  ...
  async function loadTagger(e) {
    if (framerate.validity.valid && inputtags.validity.valid) {
      ...
      videotagging.inputtagsarray = $("#inputtags")
        .val()
        .replace(/\s/g, "")
        .split(",");
      videotagging.video.currentTime = 0;
      videotagging.framerate = $("#framerate").val();
      videotagging.camera = $("#cameratype").val();
      ...
      if (isDir) {
        ...
        if (isRecords)
          $("title").text(
            `Image Tagging from Records Job: ${path.dirname(pathName)}`
          ); //set title indicator
        ...
        if (isRecords) {
          ...
          videotagging.recordlist = await Promise.all(recordlist);
          console.log(
            videotagging.imagelist.map(record => pathName + path.sep + record)
          );
          videotagging.currTFRecord = videotagging.recordlist[0];
        } else {
          videotagging.imagelist = files.filter(function(file) {
        ...
        if (videotagging.imagelist.length) {
          ...
          if (!isNaN(Array.from(visitedFrames)[0])) {
            ...
            visitedFrames = new Set(
              Array.from(visitedFramesNumber).map(
                frame => videotagging.imagelist[parseInt(frame)]
              )
            );
            ...
            Object.keys(videotagging.inputframes).map(function(key, index) {
              videotagging.inputframes[
                videotagging.imagelist[key].split(path.sep).pop()
              ] = videotagging.inputframes[key];
          ...
          $("#video-tagging").on("stepFwdClicked-AfterStep", () => {
            ...
            if (videotagging.currTFRecord) {
              if (!visitedFrames.has(videotagging.getCurrentFrameId()))
                getRegionsFromRecord(videotagging.currTFRecord);
              $("title").text(
                `Image Tagging from Records Job: ${path.basename(
                  videotagging.imagelist[videotagging.imageIndex]
                )}`
              );
            } else
              $("title").text(
                `Image Tagging Job: ${path.basename(videotagging.curImg.src)}`
              );
          ...
          alert(
            "No files were in the selected directory. Please choose an Image directory."
          );
          return folderSelected();
        }
      } else {
        $("title").text(
          `Video Tagging Job: ${path.basename(
            pathName,
            path.extname(pathName)
          )}`
        ); //set title indicator
...
function getRegionsFromRecord(tfRecord) {
  if (videotagging.sourceWidth == 0 || videotagging.sourceHeight == 0) {
    videotagging.sourceWidth =
      tfRecord.features.feature["image/width"].int64List.value[0];
    videotagging.sourceHeight =
      tfRecord.features.feature["image/height"].int64List.value[0];
  }
  ...
  for (
    let i = 0;
    i <
    tfRecord.features.feature["image/object/bbox/xmin"].floatList.value.length;
    i++
  ) {
    let x1 =
      tfRecord.features.feature["image/object/bbox/xmin"].floatList.value[i] *
      videotagging.sourceWidth *
      widthRatio;
    let y1 =
      tfRecord.features.feature["image/object/bbox/ymin"].floatList.value[i] *
      videotagging.sourceHeight *
      heightRatio;
    let x2 =
      tfRecord.features.feature["image/object/bbox/xmax"].floatList.value[i] *
      videotagging.sourceWidth *
      widthRatio;
    let y2 =
      tfRecord.features.feature["image/object/bbox/ymax"].floatList.value[i] *
      videotagging.sourceHeight *
      heightRatio;
    videotagging.createRegion(x1, y1, x2, y2);

    console.log(
      `decoded: ${decode_Uint8(
        tfRecord.features.feature["image/object/class/text"].bytesList.value[i]
      )}`
    );
    videotagging.addTagsToRegion([
      decode_Uint8(
        tfRecord.features.feature["image/object/class/text"].bytesList.value[i]
      )
    ]);
    ...
    if (!example) {
      ...
      getDataUri(recordPath)
        .then(data => {
          ...
          builder.setBinaries("image/key/sha256", [
            encode_Uint8(CryptoJS.SHA256(data).toString(CryptoJS.enc.Base64))
          ]);

          builder.setBinaries("image/encoded", [data]);

          if (videotagging.currTFRecord)
            builder.setBytes("image/format", [
              encode_Uint8(
                videotagging
                  .getCurrentFrameId()
                  .split(".")
            ...
            builder.setBinaries("image/format", [
              encode_Uint8(
                videotagging
                  .getCurrentFrameId()
                  .split(".")
          ...
          builder.setFloats(
            ...
            xmin.map(x => x / widthMult)
          );
          builder.setFloats(
            "image/object/bbox/ymin",
            ymin.map(y => y / heightMult)
          );
          builder.setFloats(
            "image/object/bbox/xmax",
            xmax.map(x => x / widthMult)
          );
          builder.setFloats(
            "image/object/bbox/ymax",
            ymax.map(y => y / heightMult)
          );
          builder.setBinaries(
            "image/object/class/text",
            tags.map(tag => encode_Uint8(tag))
          );
          builder.setIntegers(
            "image/object/class/label",
            tags.map(tag => videotagging.inputtagsarray.indexOf(tag))
          );
...
function save() {
  ...
  if (!saveLock) {
    saveLock = true;
    fs.writeFile(
      `${videotagging.src}.json`,
      JSON.stringify(saveObject, null, "\t"),
      () => {
    ...
    if (videotagging.currTFRecord) {
      ...
      videotagging.sourceHeight =
        videotagging.currTFRecord.features.feature[
          "image/height"
        ].int64List.value[0];
      videotagging.sourceWidth =
        videotagging.currTFRecord.features.feature[
      ...
      videotagging.currTFRecord.features.feature[
        "image/object/bbox/xmin"
      ].floatList.value = xmin;
      videotagging.currTFRecord.features.feature[
        "image/object/bbox/xmax"
      ].floatList.value = xmax;
      videotagging.currTFRecord.features.feature[
        "image/object/bbox/ymin"
      ].floatList.value = ymin;
      videotagging.currTFRecord.features.feature[
        "image/object/bbox/ymax"
      ].floatList.value = ymax;
      videotagging.currTFRecord.features.feature[
        "image/object/class/text"
      ].bytesList.value = tags;
      videotagging.recordlist[videotagging.imageIndex] =
        videotagging.currTFRecord;
...
function deleteFrame() {
  ...
  if (
    !confirm(
      "This will delete the image from disk and remove it's tags from the save file.\nAre you sure you want to delete this image?"
    )
  )
    return;
