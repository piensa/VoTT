...
const isChangedIds = process.argv[3] === "true" || false;
...
const modifyBoxId = (boxId, quadrant) => {
  const step = 3;
  switch (quadrant) {
    case "left":
      return boxId * step - 2;
...
const generateOriginalAnnotations = file => {
  ...
  const newFrames = frameList.reduce(
    (acc, frameNumber) => {
      const newFrame = vottfile.frames[frameNumber]
        ...
        .reduce(
          (acc, box) => {
            ...
            if (isChangedIds) {
              box.boxId = modifyBoxId(box.boxId, quadrant);
            }
  ...
  const leftVott = { ...vottfile, frames: newFrames.left, camera: "Left" };
  const cenVott = { ...vottfile, frames: newFrames.center, camera: "Center" };
  const rightVott = { ...vottfile, frames: newFrames.right, camera: "Right" };
